...
def read_configs(client: Client = Depends(get_supabase_client)):
    resp = client.table("pos_config").select("*").execute()
    return resp.data
...
def read_sessions(client: Client = Depends(get_supabase_client)):
    resp = client.table("pos_session").select("*").execute()
    return resp.data
...
def read_orders(client: Client = Depends(get_supabase_client)):
    resp = client.table("pos_order").select("*, lines:pos_order_line(*)").execute()
    return resp.data
