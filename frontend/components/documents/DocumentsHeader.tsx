...
import { Search, LayoutGrid, List, FileText, Plus, Upload } from "lucide-react";
...
export default function DocumentsHeader() {
    ...
    const navItems = [
        ...
        { name: "Shared", href: "/documents/shared" },
        { name: "Recent", href: "/documents/recent" },
        { name: "Trash", href: "/documents/trash" },
    ];
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/documents" && pathname.startsWith(item.href))
                                        ? "text-white"
