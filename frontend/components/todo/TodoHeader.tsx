...
import { Search, LayoutGrid, List, CheckSquare, Plus } from "lucide-react";
...
export default function TodoHeader() {
    ...
    const navItems = [
        ...
        { name: "Completed", href: "/todo/completed" },
    ];
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/todo" && pathname.startsWith(item.href))
                                        ? "text-white"
