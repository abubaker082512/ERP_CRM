...
import { Search, LayoutGrid, List, BookOpen, Plus } from "lucide-react";
            ...
            <div className="flex items-center justify-between px-4 py-2">
                <div className="flex items-center gap-4">
                    <Link href="/" className="flex items-center gap-2 text-xl font-semibold text-gray-200 hover:text-white">
                        <div className="bg-teal-500 w-7 h-7 flex items-center justify-center rounded text-sm font-bold text-white">
                            <BookOpen size={16} />
                        </div>
                        Knowledge
                    </Link>
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/knowledge" && pathname.startsWith(item.href))
                                        ? "text-white"
