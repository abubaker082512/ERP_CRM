...
export default function EmployeesHeader({ onNewClick }: { onNewClick?: () => void }) {
            ...
            <div className="flex items-center justify-between px-4 py-2">
                <div className="flex items-center gap-4">
                    <Link href="/" className="flex items-center gap-2 text-xl font-semibold text-gray-200 hover:text-white">
                        <div className="bg-purple-600 w-7 h-7 flex items-center justify-center rounded text-sm font-bold text-white">
                            <Users size={16} />
                        </div>
                        Employees
                    </Link>
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== '/employees' && pathname.startsWith(item.href)) ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                                    }`}
                            >
                                {item.name}
                            </Link>
