...
import { Search, LayoutGrid, List, MessageSquare, Plus } from "lucide-react";
            ...
            <div className="flex items-center justify-between px-4 py-2">
                <div className="flex items-center gap-4">
                    <Link href="/" className="flex items-center gap-2 text-xl font-semibold text-gray-200 hover:text-white">
                        <div className="bg-indigo-600 w-7 h-7 flex items-center justify-center rounded text-sm font-bold text-white">
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/surveys" && pathname.startsWith(item.href))
                                        ? "text-white"
                ...
                <div className="flex-1 max-w-xl mx-8">
                    <div className="relative flex items-center">
                        ...
                        <input
                            ...
                            className="w-full bg-[#0F172A] border border-gray-600 rounded px-10 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-indigo-500"
                        />
                    </div>
                </div>
