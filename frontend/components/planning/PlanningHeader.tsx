...
import { Search, LayoutGrid, List, CalendarRange, Plus } from "lucide-react";
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/planning" && pathname.startsWith(item.href))
                                        ? "text-white"
                ...
                <div className="flex-1 max-w-xl mx-8">
                    <div className="relative flex items-center">
                        ...
                        <input
                            ...
                            className="w-full bg-[#0F172A] border border-gray-600 rounded px-10 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-cyan-500"
                        />
                    </div>
                </div>
