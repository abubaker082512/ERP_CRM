...
import { Search, Settings, MessageSquare, Bot, LayoutGrid, List, Filter, ChevronDown, Calculator } from 'lucide-react';

export default function AccountingHeader({ onNewClick }: { onNewClick?: () => void }) {
    ...
    const navItems = [
        ...
        { name: 'Accounting', href: '/accounting/journal-entries' },
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== '/accounting' && pathname.startsWith(item.href)) ? 'text-white' : 'text-gray-400 hover:text-gray-200'
                                    }`}
                            >
                                {item.name}
                            </Link>
