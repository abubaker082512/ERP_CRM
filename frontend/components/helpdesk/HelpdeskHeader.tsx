...
import { Search, LayoutGrid, List, MessageSquare, Bot, Plus } from "lucide-react";

export default function HelpdeskHeader() {
    const pathname = usePathname();

    const navItems = [
        { name: "Dashboard", href: "/helpdesk" },
                    ...
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/helpdesk" && pathname.startsWith(item.href))
                                        ? "text-white"
