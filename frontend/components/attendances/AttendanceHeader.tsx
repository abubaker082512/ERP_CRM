...
import { Search, LayoutGrid, List, UserCheck, Plus } from "lucide-react";
            ...
            <div className="flex items-center justify-between px-4 py-2">
                <div className="flex items-center gap-4">
                    <Link href="/" className="flex items-center gap-2 text-xl font-semibold text-gray-200 hover:text-white">
                        <div className="bg-orange-400 w-7 h-7 flex items-center justify-center rounded text-sm font-bold text-white">
                            <UserCheck size={16} />
                        </div>
                        Attendances
                    </Link>
                    <nav className="flex items-center gap-6 ml-4">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`text-sm font-medium transition-colors ${pathname === item.href || (item.href !== "/attendances" && pathname.startsWith(item.href))
                                        ? "text-white"
                ...
                <div className="flex-1 max-w-xl mx-8">
                    <div className="relative flex items-center">
                        ...
                        <input
                            type="text"
                            placeholder="Search..."
                            className="w-full bg-[#0F172A] border border-gray-600 rounded px-10 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-orange-500"
                        />
                    </div>
                </div>
