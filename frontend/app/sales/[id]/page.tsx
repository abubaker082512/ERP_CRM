...
export default function SalesOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      const res = await fetchAPI(`/sales/${params.id}/confirm`, { method: "POST" });
      if (res.ok) {
        const d = await res.json();
        setOrder(d);
        showToast("Order confirmed! Invoice and delivery created.");
  ...
  const handleDelete = async () => {
    ...
    await fetchAPI(`/sales/${params.id}`, { method: "DELETE" });
    router.push("/sales");
  };
  ...
  return (
    <div className="min-h-screen p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-6 text-sm text-gray-400">
        <Link href="/sales" className="flex items-center gap-1 hover:text-purple-400 transition-colors">
            ...
            <tbody>
              {lines.map((line: any) => (
                <tr key={line.id} className="border-b border-white/5 hover:bg-white/3">
                  ...
                  <td className="py-3 text-right text-gray-300">{line.product_uom_qty || line.qty || 1}</td>
